...
Evals (`evals/`) are pair→merge and end-to-end: article pairs through the production extraction and matching, asserting what should merge. They make LLM calls and are expected to be red while the extraction prompt is untuned. The only extraction convention beyond the shape is the active-voice rule — there are no annotation guidelines to maintain.
